...
def receivesingle_icmpping(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        trans_time = struct.unpack("d", received_Packet[28:28 + bytesIndouble])[0]
...
def checksum(string):
    ...
    if countTo < len(string):
        count_sum = count_sum + ord(string[len(string) - 1])
        count_sum = count_sum & 0xffffffff
...
def sendsingle_icmpping(mySocket, destination_add, ID):
    ...
    data2=(192-struct.calcsize("d"))*"Q"
    data = struct.pack("d", time.time())+bytes(data2.encode('utf-8'))
